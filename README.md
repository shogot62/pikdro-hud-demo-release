...
PikDro HUD Demo は、Android向けのデモ版APKアプリです。

Uber Driver などの配達オファー画面に表示される金額・距離・予定時間などをもとに、距離単価や時間単価などの判断補助情報をHUDとして表示します。

本アプリは、配達中の情報把握や判断を支援する認知アクセシビリティ補助ツールであり、配達サービス各社の公式アプリではありません。
また、配達サービス各社と提携、承認、認定、協賛されたものではありません。

このリポジトリは、PikDro HUD Demo の APK配布専用リポジトリです。
